...
place_hier_ca <- read.csv("https://www2.census.gov/geo/docs/reference/codes/PLACElist.txt", sep="|", header=TRUE) %>%
  ...
  mutate(placefp = if_else(nchar(placefp) == 3, str_pad(placefp, width = 4, pad = "0"),
                           if_else(nchar(placefp) == 4, str_pad(placefp, width = 5, pad = "0"), placefp)))%>%
  mutate(placefp = if_else(nchar(placefp) == 4, str_pad(placefp, width = 5, pad = "0"), placefp))
