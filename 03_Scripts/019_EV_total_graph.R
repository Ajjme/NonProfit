#https://cleancities.energy.gov/accomplishments/
 # Build a shiny app like this on with tabs
  ...
  ZEV_sales_city_standardized_elect <- readRDS( file = "./06_Reports_Rmd/ZEV_sales_city_standardized_ev.rds")
  ...
  scores_ev <- all_joined %>% 
    select(city, number_of_buses, ev_per_new_vehicle_scaled, chargers_per_total_vehicle_scaled) %>% 
...
saveRDS(  scores_ev, file = "./06_Reports_Rmd/scores_ev.rds")
  ...
  plotly_obj_scores_ev <- scores_ev %>% 
    ...
    add_trace(y = ~ev_per_new_vehicle_scaled, name = "EVs per Vehicle Score", 
